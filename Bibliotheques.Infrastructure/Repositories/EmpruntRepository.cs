...
public class EmpruntRepository(BibliothequeDbContext c) : Repository<Emprunt>(c), IEmpruntRepository {
    ...
    public Task<Exemplaire?> GetAvailableExemplaireAsync(int livreId)=>Context.Exemplaires
        .Where(x=>x.LivreId==livreId && x.Etat=="Disponible" && !x.Emprunts
        .Any(e=>e.DateRetour==null)).FirstOrDefaultAsync();
}
